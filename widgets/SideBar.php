            ...
            foreach ($this->links as $link)
            {
                ...
                else
                {
                    ...
                    if (!isset($link['options'])){
                        $link['options']['class'] = 'nav-link';
                    }

                    $options = ['class'=>'nav-item'];

                    $route = array_filter(explode('/', $link['url'][0]));
                    if (!empty($route))
                        $options['class'] .= Yii::$app->controller->id == current($route) ? ' active' :'';


                    $icon = isset($link['icon']) ? "<i class='pe-7s-{$link['icon']}'></i>" : '';
                    $url = Html::a("{$icon} <p>{$link['title']}</p>", $link['url'], $link['options']);

                    $content.= Html::tag('li', $url, $options);
                }
            }
